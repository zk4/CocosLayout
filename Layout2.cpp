...
void Layout2::DrawSpace ( CCNode* _target ) {
    CCPoint target_world_pos = _target->convertToWorldSpace ( CCPointZero );
    CCSize s = CCDirector::sharedDirector()->getWinSize();
    CCPoint target_world_left_button, target_world_right_top, target_world_center;
    Layout2::GetWorldRect ( _target, target_world_left_button, target_world_right_top, target_world_center );
    ccDrawColor4F ( Tools::RandomBetween ( 0, 256 ) / 256.0f, 0, 0, 1 );
    drawLine ( ccp ( target_world_left_button.x, 0 ), ccp ( target_world_left_button.x, s.height ), 5 );
    ccDrawColor4F ( Tools::RandomBetween ( 0, 256 ) / 256.0f, 0, 0, 1 );
    drawLine ( ccp ( 0, target_world_left_button.y ), ccp ( s.width, target_world_left_button.y ), 5 );
    ccDrawColor4F ( 1, 1, 1, 1 );
    drawLine ( ccp ( target_world_right_top.x, 0 ), ccp ( target_world_right_top.x, s.height ), 5 );
    drawLine ( ccp ( 0, target_world_right_top.y ), ccp ( s.width, target_world_right_top.y ), 5 );
    ccDrawColor4F ( 0, 1, 0, 1 );
    //center
    drawLine ( ccp ( target_world_center.x, 0 ), ccp ( target_world_center.x, s.height ), 2 );
    drawLine ( ccp ( 0, target_world_center.y ), ccp ( s.width, target_world_center.y ), 2 );
}
...
void DebugDrawLayer::LazyInit() {
    if ( DebugDrawLayer::s_drawLayer == NULL ) {
        ...
        CC_NOTIFICATION->addChild ( s_drawLayer );
    }
}
...
void DebugDrawLayer::AddRect ( CCRect r, ccColor4F color /*= ccc4f (1,1,1,1)*/ ) {
    ...
    AST ( s_id != INT_MAX && "you are awsome!" );
